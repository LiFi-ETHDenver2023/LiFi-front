...
export default function ProfileTemplate() {
  ...
  const [user, setUser] = React.useState({
    ...
    description: 'Hello, world! I am staykeen.',
  });
  ...
  const handleClaim = async (id: number) => {
    try {
      ...
      if (serverResponse && hostInfoResponse) {
        if (confirm('[Web3Auth] Redeem the staking reward for the completed challenge.')) {
          getUser();
          getRecords();
        }
      }
  ...
  return (
    <Container roomImageUrl={isUpdatedRoom ? user.roomImageUrl : user.roomImageUrl}>
